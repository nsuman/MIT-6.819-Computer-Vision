...
for i = 2:nrows-1
    for j = 2:ncols-1
        if ground(i,j)==1
            ...
            % neirborhood)            
            nx = sum(sum(dmdx(i-1:i+1,j-1:j+1).*edges(i-1:i+1,j-1:j+1)))/edgesum;
            ny = sum(sum(dmdy(i-1:i+1,j-1:j+1).*edges(i-1:i+1,j-1:j+1)))/edgesum;
            orientation = atan2(nx, ny);

            if contact_edges(i,j)==1
                % Y = 0
                c = c+1;   % increment constraint counter
                Aij(:,:,c) = [0 0 0; 0 1 0; 0 0 0]; % kernel
            ...
            if horizontalsum>0 && groundsum==0 && vericalsum==0
                % dY/dt = 0
                c = c+1; % increment constraint counter
                Aij(:,:,c) = nx * [-1 -2 -1; 0 0 0; 1 2 1]/8 - ny * [-1 -2 -1; 0 0 0; 1 2 1]'/8;
                ii(c) = i; 
                jj(c) = j;
                b(c)  = 0;
            end
